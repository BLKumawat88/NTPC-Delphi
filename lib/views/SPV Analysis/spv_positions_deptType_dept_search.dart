...
class SPVDeptTypeDeptSearch extends StatefulWidget {
  SPVDeptTypeDeptSearch({super.key});
  static const _chars =
      'AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz1234567890';

  @override
  State<SPVDeptTypeDeptSearch> createState() => _SPVDeptTypeDeptSearchState();
}

class _SPVDeptTypeDeptSearchState extends State<SPVDeptTypeDeptSearch> {
  ...
  applyFilter(context) {
    ...
    Map<String, String> spvAnalysisProjectFilterData = {
      ...
      'SearchDeptTypeID': provider.spvProjectDepTypeID.toString(),
    ...
    provider.setSpvAnalysisRequiredFilters(
        context, 4, spvAnalysisProjectFilterData, false, false);
  }
  ...
  String getRandomString(int length) => String.fromCharCodes(Iterable.generate(
      length,
      (_) => SPVDeptTypeDeptSearch._chars
          .codeUnitAt(rnd.nextInt(SPVDeptTypeDeptSearch._chars.length))));
  ...
  downloadExcelFile(AllInProvider provider, context) async {
    final excel = Excel.createExcel();
    final Sheet sheet = excel[excel.getDefaultSheet()!];

    List excelData = [
      {
        "svp_Department_Name": "SPV Dept.",
        "total": "S",
        "p_Total": "P",
        "t_Total": "V",
      },
      ...provider.spvAnalysisProjectDeptTypeDeptData
    ];
    ...
    for (int i = 0; i < excelData.length; i++) {
      sheet
          ...
          .value = '${excelData[i]['svp_Department_Name']}';
  ...
  returnOflfineSTotal(context) {
    AllInProvider provider = Provider.of(context, listen: false);
    int total = provider.spvAnalysisProjectDeptTypeDeptData
        .fold(0, (int sum, item) => sum + item['total'] as int);
    return total.toString();
  }

  returnOflfinePTotal(context) {
    AllInProvider provider = Provider.of(context, listen: false);
    int total = provider.spvAnalysisProjectDeptTypeDeptData
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: Colors.transparent,
        title: Text(
          "${provider.selectedRegionName} -- ${provider.selectedProjectName} -- ${provider.selectedProjectDeptTypeName}",
          style: TextStyle(color: Colors.black),
        ),
        leading: InkWell(
          onTap: () {
                ...
                Consumer<AllInProvider>(builder: (context, person, _) {
                  return Container(
                    ...
                    child: DataTable(
                      ...
                      columns: <DataColumn>[
                        DataColumn(
                          label: returnTableColumnLable('SPV Dept.'),
                        ),
                        DataColumn(
                          ...
                          ? <DataRow>[
                              ...
                              DataRow(
                                ...
                                cells: [
                                  ...
                                  DataCell(
                                    Center(
                                      child: Text(returnOflfineSTotal(context)),
                                    ),
                                  ),
                                  DataCell(
                                    Center(
                                      child: Text(returnOflfinePTotal(context)),
                                    ),
                                  ),
                                  DataCell(
                                    Center(
                                      child: Text(returnOflfineTTotal(context)),
                                    ),
                                  ),
                                ],
                              )
